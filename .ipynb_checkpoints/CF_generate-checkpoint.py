...
def CF_generate(dataset, model_name, CF_method='NG', AE_name='FCN_AE', vis_flag=False, device='cuda:0'):
    ...
    if CF_method == 'NG':  # TODO refractor so that we can put cuda: as device correctly
        ...
        exp_model: CFs.TSEvo = CFs.TSEvo(model=model, data=(test_x, np.array(y_pred)), mode='feat', backend='PYT', epochs=500)  # TODO check the cuda was added in the right way
    elif CF_method == 'COMTE':
        exp_model: CFs.COMTECF = CFs.COMTECF(model, (train_x, train_y), backend='PYT', mode='feat', method='opt')
    elif CF_method == 'SETS':
    ...
    for i in range(num_instance):
        ...
        if isinstance(exp_model, CFs.COMTECF) or isinstance(exp_model, CFs.SETSCF) or isinstance(exp_model, CFs.wCF):  # we keep the target as the second largest prediction
            ...
            CF, pred_CF = exp_model.explain(orig, pred_label)  # TODO Check again for NUNCF and NG
