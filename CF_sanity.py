...
from utils.data_util import read_UCR_UEA, get_UCR_UEA_sets, save_result_JSON, get_result_JSON,sample_indices_by_label
from utils.model_util import model_init
from utils.visual_util import visualize_TSinterpret
from quantative.metric import proximity, sparsity, mean_absolute_error, generate_metric_stat
...
def CF_generate(dataset, model_name, CF_method='NG', AE_name='FCN_AE', vis_flag=False, device='cuda:0', num_instance=250):
    model_dataset_path = f'../models/{model_name}/{dataset}'
    CF_path = f'../CF_result/{CF_method}/{model_name}/{dataset}_sanity'
    ...
    if CF_method == 'NG':
        ...
        exp_model: CFs.NGCF = CFs.NGCF(model, (train_x, train_pred), backend='PYT', mode='feat',distance_measure="euclidean", method='NUN_CF', max_iter=input_size, device=device)
        ...
        exp_model: CFs.COMTECF = CFs.COMTECF(model, (train_x, train_pred), backend='PYT', mode='feat', method='opt',number_distractors=1, device=device)
    elif CF_method == 'SETS':
    ...
    random_selection = sample_indices_by_label(test_pred,num_instance)
    ...
    for i in range(num_instance) :
        ...
        CF, pred_CF = explain_with_timeout(exp_model, orig, pred_label)

        durations = time.time() - start
        print(CF, pred_CF, durations)
    ...
    np.save(f'{CF_path}/test_y.npy', np.argmax(test_y, axis=1))
    np.save(f'{CF_path}/generation_times.npy', np.array(generation_times))
    np.save(f'{CF_path}/random_selection.npy', np.array(random_selection))
...
def generate_CF(CF_name, model_name, dataset_choice, device: str = 'cuda:0', start_per: float = 0.0, end_per: float = 1.0):
    ...
    for model_name in models:
        ...
        for i in range(start, end):
            CF_classification = 'CF/'+model_name+'/'+CF_name+'_copy'
            dataset = datasets[i]
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--CF_name', type=str, default='TSEvo', help='classification model name') #NG NUN
    # parser.add_argument('--model_name', type=str, default='FCN', help='classification model name')
    # parser.add_argument('--dataset_choice', type=str, default='multiclass_uni', help='dataset name')
    ...
    # save_result_JSON(method_name='TSEvo_sampling_check', record_dict=TSEvo_sampling_check)
    mp.set_start_method('spawn')
    CF_generate('HandOutlines', 'MLP', CF_method='wCF',device='cuda:2', num_instance=160)
        # save_result_JSON(method_name='TSEvo_sampling_check', record_dict=TSEvo_sampling_check)