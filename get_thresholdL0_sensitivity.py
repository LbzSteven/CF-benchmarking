...
def count_total_different_segments(orig:np.array, cf:np.array,min_gap=2):
    ...
    for dim in range(orig.shape[0]):
        ...
        for is_different in differences:
            if is_different:
...
def threshold_replace(orig, cf, threshold=0.001,verbose=False):
    ...
    L0_threshold = np.sum(diff >= drange) / orig.shape[1]
    if verbose:
...
def threshold_sparsity(orig, cf, cf_pred, model, threshold=0.001,min_gap=2):
    cf_modified, L0_threshold, count_tiny_modification = threshold_replace(orig, cf, threshold=threshold)
    sensitivity = 0
    num_segment = count_total_different_segments(orig,cf,min_gap)
...
def threshold_sparsity_on_dataset(tx_valid, cfs, cf_preds, model, threshold=0.001,min_gap=2):
    L0s = []
    sensitivities = []
    ...
    for i in range(num_valid):
        ...
        cf_pred = cf_preds[i]
        L0_threshold, sens,num_segment,num_segment_modi = threshold_sparsity(orig, cf, cf_pred, model, threshold=threshold)
...
def compute_threshold_L0_datasets(model_name,CF_name,datasets_selection,threshold=0.001,min_gap=2):
    threL0_results = []
    ...
    for dataset in datasets:
        ...
        if results[-1] == 0.0 or 'Time out 10 consecutive time' in str(results) or 'NotEvaluate' in str(results):
            pbar.update(1)
            continue
        ...
        marker = marker + '_' + str(min_gap) if min_gap > 1 else marker
        np.save(os.path.join(CF_path, f'num_segments{thresmarker}_{marker}.npy'), np.array(num_segments))
        np.save(os.path.join(CF_path, f'num_segments_modi{thresmarker}_{marker}.npy'), np.array(num_segment_modis))
...
def compute_threshold_L0_models(model_name, CF_names=None, datasets_selection='selected_uni',threshold=0.001):
    if CF_names is None:
        CF_names = ['NUN_CF', 'wCF','NG', 'TSEvo', 'SETS']
    results = []
    for CF_name in CF_names:
        results +=compute_threshold_L0_datasets(model_name, CF_name, datasets_selection,threshold)
    df_threL0_sensitivity = pd.DataFrame(results,
                                         columns=['model_name','CF_name', 'dataset_name', 'L0thre', 'L0thre_std',
                                                          'sensitivity','seg_mean','seg_std','seg_modi','seg_modi_std'])
...
if __name__ =='__main__':
    ...
    model_names = ['InceptionTime', 'MLP','FCN']#['InceptionTime', 'MLP','FCN']
    # CF_names = ['COMTE','wCF','TSEvo','SETS'] #'NUN_CF'
    for model_name in model_names:
        CF_names = ['NUN_CF','wCF','TSEvo','SETS'] if model_name == 'MLP' else  ['NUN_CF','NG','wCF','TSEvo','SETS']
        df_threL0_sensitivity:pd.DataFrame = compute_threshold_L0_models(model_name, CF_names, datasets_selection,threshold)
        marker = '' if threshold == 0.001 else str(threshold)
        marker = marker + '_' + str(min_gap) if min_gap > 1 else marker
