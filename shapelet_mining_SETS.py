...
def SETS_mine(dataset, max_len=None, contract_time_per_dim=None, UCR_UEA_dataloader=None):
    ...
    SETS = CFs.SETSCF(model,
                      ...
                      fit_shapelets=False)
    SETS.fit(occurrences_threshhold=1e-1, remove_multiclass_shapelets=True)
...
def mining_shapelets_for_SETS(dataset_choice, start_per: float = 0.0, end_per: float = 1.0):
    ...
    for i in range(start, end):
        ...
        method_record = get_result_JSON(JSON_name)

        if method_record[dataset] == 'NotTrained':
            results = SETS_mine(dataset, max_len=None, contract_time_per_dim=None, UCR_UEA_dataloader=UCR_UEA_dataloader)

            method_record = get_result_JSON(JSON_name)
            method_record[dataset] = results
            save_result_JSON(method_name=JSON_name, record_dict=method_record)

        else:
            print(f'{JSON_name} on {dataset} was already fitted')
...
if __name__ == '__main__':
    ...
    results = SETS_mine(args.dataset_choice, max_len=None, contract_time_per_dim=None, UCR_UEA_dataloader=UCR_UEA_datasets())
    method_record = get_result_JSON('Shapelets')
    method_record[args.dataset_choice] = results
    save_result_JSON(method_name='Shapelets', record_dict=method_record)